...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const userConf = await UserConfig.findOne({ userId: interaction.user.id, guildId });
        const masterName = userConf ? userConf.preferredName : 'Master';

        let item = await Item.findById(itemIdOrName) || await Item.findOne({ userId: interaction.user.id, guildId, activeSeq: parseInt(itemIdOrName), isArchived: false });
        if (!item) return interaction.reply({ content: 'Item not found.', ephemeral: true });

        const frequency = await Frequency.findOne({ guildId, name: freqName });
        if (!frequency) return interaction.reply({ content: 'Rhythm not found.', ephemeral: true });
        ...
        return interaction.reply({ content: ai.getMoveMessage(freqName, masterName), ephemeral: true });
    }
};