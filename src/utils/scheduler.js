...
async function checkReminders(client) {
    console.log("[Scheduler] Checking for due items...");
    try {
        const now = new Date();
        
        // Anti-Deadlock: Reset items that were missed/forgotten
        const yesterday = new Date(now.getTime() - 24 * 60 * 60 * 1000);
        await Item.updateMany(
            { awaitingReview: true, nextReminder: { $lte: yesterday } },
        ...
        for (const userId of Object.keys(userItems)) {
            ...
            for (let i = 0; i < items.length; i += chunkSize) {
                ...
                embeds[0].setTitle(`📅 Daily Protocol for ${masterName || 'Master'}`)
...
async function startScheduler(client) {
    ...
    console.log(`[Scheduler] Starting internal clock: "${cronTime}" (IST)`);
